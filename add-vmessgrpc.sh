...
} 2>&1 | tee -a /tmp/created-vmessgrpc.log
sed -i 's/ID : \(.*\)/ID : <code>\1<\/code>/g' /tmp/created-vmessgrpc.log
sed -i 's/LINK gRPC TLS : \(.*\)/LINK gRPC TLS : <code>\1<\/code>/g' /tmp/created-vmessgrpc.log
KEY=$(cat /etc/william/profile/private/key 2>/dev/null)
CHATID=$(cat /etc/william/profile/private/chatid 2>/dev/null)
message=$(cat /tmp/created-vmessgrpc.log)
curl -s -X POST https://api.telegram.org/bot${KEY}/sendMessage -F chat_id="${CHATID}" -F text="
$message" -F parse_mode=html > /dev/null 2>&1
rm -rf /tmp/created-vmessgrpc.log
